"""POST /predict/video — video deepfake detection endpoint.

HTTP concerns only: validation, persistence and error translation. All
analysis is delegated to `app.services.video`, which itself reuses the
image AI pipeline frame by frame.
"""
...
from app.models.schemas import VideoPredictionResponse
from app.services.ai.response_builder import build_error_response
from app.core.config import settings
from app.services.video import predict_video_file
...
from app.utils.validators import validate_upload_envelope, validate_video_path
...
async def predict_video_endpoint(file: UploadFile = File(...)):
    ...
    envelope_error = validate_upload_envelope(
        file.filename, file.content_type, "video")
    if envelope_error:
    ...
    if not validation.is_valid:
        message = validation.error or "Invalid video."
    ...
    logger.info("%s: %s (%.1f MB)", LogEvent.VALIDATION_PASSED,
                file.filename, validation.size_bytes / (1024 * 1024))

    digest = stored.sha256
    saved_path = stored.path
    logger.info("%s: %s -> %s", LogEvent.VIDEO_UPLOADED, file.filename, saved_path)

    try:
        return predict_video_file(saved_path, filename=file.filename,
                                  sha256=digest)
        ...
        return _error(
            ...
            "Video analysis failed. Please try a different file.",
            filename=file.filename,
        )
